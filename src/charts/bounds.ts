...
export function computeBounds(all: TimedForecast[], o: BoundsOverrides = {}): AxisBounds {
  if (!all.length) {
  ...
  let tMin = Math.floor((Math.min(...temps) - TEMP_PAD) / TEMP_STEP) * TEMP_STEP;
  let tMax = Math.ceil((Math.max(...temps) + TEMP_PAD) / TEMP_STEP) * TEMP_STEP;
  ...
  const precipMax = Math.max(
    PRECIP_FLOOR,
    Math.ceil(Math.max(...precip) / PRECIP_STEP) * PRECIP_STEP,
  );
